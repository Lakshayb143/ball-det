...
PLAYER_MODEL_PATH = "players.pth" # <-- UPDATE THIS PATH
IMAGE_DIR_PATH = "snmot196"
OUTPUT_PATH = "output_v9_6.mp4"
    ...
    def run(self):
        ...
        with open(self.args.prediction_path, "w") as pred_file:
            for frame_count, image_path in enumerate(self.image_files):
                ...
                # --- V9_6: Detect both players and balls ---
                ...
                ball_detections = self.model.predict(frame, confidence=self.args.ball_confidence)
                ball_detections = ball_detections[ball_detections.class_id == BALL_CLASS_ID]
                
                # --- V9_6: Pre-filter ball detections using the Exclusion Zone ---
                valid_ball_indices = []
                ...
                # --- Use the BallTracker on the filtered list ---
                ...
                if measurement_abs is not None:
                    ...
                    self.track_initialized = False
                    # Do not reset ball_tracker buffer if we lose track, it helps reacquire
                    self.optical_flow_points_rel = None
                    self.prev_position_abs = None
