...
IMAGE_DIR_PATH = "img1"
OUTPUT_PATH = "output_v17_5.mp4"
PREDICTION_FILE_PATH = "predictions_v17_5.txt"
...
OCCLUDED_CONFIRMATION_THRESHOLD = 2 # Reduced for faster occlusion detection
...
GATE_SIZE_AIR = 150.0
EXCLUSION_CONFIDENCE_PENALTY = 0.0# Re-added this parameter
...
class VideoProcessor:
    ...
    def _manage_state_transitions(self, measurement_abs, annotation_sv, predicted_pos_abs, player_detections, pitch_mask, coord_transform):
        ...
        if self.occluded_streak >= OCCLUDED_CONFIRMATION_THRESHOLD: self.ball_state = BallState.OCCLUDED
        elif self.air_streak >= AIR_CONFIRMATION_THRESHOLD: self.ball_state = BallState.IN_AIR
        elif self.ground_streak >= GROUND_CONFIRMATION_THRESHOLD: self.ball_state = BallState.ON_GROUND
    ...
    def run(self):
        ...
        with open(self.args.prediction_path, "w") as pred_file:
            for frame_count, image_path in enumerate(self.image_files, 1):
                ...
                # --- V17_5: Re-integrated Exclusion Zone ---
        ...
        out_writer.release()
        print("Processing complete for V17_5.")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="V17_5: Stable tracker with exclusion zone re-integrated.")
