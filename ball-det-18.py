...
PLAYER_MODEL_PATH = "players.pth"
IMAGE_DIR_PATH = "snmot196"
OUTPUT_PATH = "output_v18.mp4"
PREDICTION_FILE_PATH = "predictions_v18.txt"
...
# --- V18: Velocity Control Parameter ---
# Factor by which to reduce velocity if a player intersection is predicted
VELOCITY_DAMPING_FACTOR = 0.5 

# --- V17 Player Occlusion Mode Parameters ---
OCCLUSION_ENTRY_THRESHOLD = 75.0 
MAX_OCCLUSION_FRAMES = 25 
MAX_LOST_FRAMES = 8 
REACQUISITION_GATE_MULTIPLIER = 1.5 

# --- Existing Tuning Parameters ---
Q_SCALE_DEFAULT = 0.5   
Q_SCALE_OCCLUDED = 5.0 
...
class VideoProcessor:
    def __init__(self, args):
        # ... (This method is unchanged from v17) ...
        self.args = args
    ...
    def _update_ball_state(self, ball_pos_abs, pitch_mask, coord_transform):
        ...
        if self.ball_state == BallState.ON_GROUND:
            ...
            else: self.on_ground_frames_count = 0
            if self.on_ground_frames_count >= ON_GROUND_CONFIRMATION_THRESHOLD: self.ball_state = BallState.ON_GROUND
    ...
    def run(self):
        ...
        with open(self.args.prediction_path, "w") as pred_file:
            for frame_count, image_path in enumerate(self.image_files, 1):
                ...
                # --- V18: Velocity Control Logic ---
                if self.track_initialized and self.ball_state != BallState.OCCLUDED:
                    # Look ahead one step
                    current_pos_abs = self.kf.x_hat[:2].flatten()
                    vx, vy = self.kf.x_hat[2:].flatten()
                    dt = self.kf.dt
                    look_ahead_pos_abs = current_pos_abs + np.array([vx*dt, vy*dt])
                    
                    # Check if look-ahead position is inside a player
                    look_ahead_pos_rel = coord_transform.abs_to_rel(np.array([look_ahead_pos_abs]))[0]
                    if is_point_in_boxes(look_ahead_pos_rel, player_detections.xyxy):
                        # If so, dampen the velocity in the KF state
                        self.kf.x_hat[2:] *= VELOCITY_DAMPING_FACTOR
                
                # --- Main Prediction and Association ---
                predicted_pos_abs = self.kf.predict() if self.track_initialized else None
                # ... (The rest of the run loop is the same as v17_corrected) ...
                if predicted_pos_abs is not None and self.ball_state != BallState.OCCLUDED:
                ...
                if measurement_abs is not None:
                    ...
                    if self.lost_frames_count > timeout: self.track_initialized = False
        ...
        out_writer.release()
        print("Processing complete for V18.")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="V18: Ball tracking with Player Occlusion and Velocity Control.")
