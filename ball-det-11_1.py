...
OUTPUT_PATH = "output_v11_1.mp4"
PREDICTION_FILE_PATH = "predictions_v11_1.txt"
...
# Confidence score of a ball detection is multiplied by this if it's inside a player box
    ...
    def run(self):
        ...
        with open(self.args.prediction_path, "w") as pred_file:
            for frame_count, image_path in enumerate(self.image_files):
                ...
                # --- ASSOCIATION with Adaptive Gate ---
                ...
                if len(ball_detections) > 0:
                    ...
                    if self.track_initialized and predicted_pos_abs is not None:
                        ...
                        # We now consider confidence in our choice, prioritizing high-confidence detections within the gate
                        valid_indices = np.where(distances < validation_gate)[0]
                        if len(valid_indices) > 0:
                            # Among valid detections, pick the one with the highest (penalized) confidence
        ...
        out_writer.release()
        print("Processing complete for V11.1.")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="V11.1: Ball tracking with player-based exclusion zones.")
